...
class StrategyStatusChecker:
    ...
    def judge(self, strategy: Strategy, flow_status: str) -> Optional[str]:
        ...
        if not (strategy.backend_data or {}).get("flow_id") and strategy.status not in (
            ...
            return StrategyStatusAnomalyReason.MISSING_FLOW_ID.label
        ...
        if strategy.status in failed_statuses:
            return StrategyStatusAnomalyReason.OPERATION_FAILED.label.format(status=strategy.status)
        return None
    ...
    def check_no_schedule_records(
        ...
        if (
            ...
            if not running_data:
                return StrategyStatusAnomalyReason.RUNNING_FLOW_NO_SCHEDULE_RECORDS.label

            # 检查是否有失败的调度记录
            failed_records = [record for record in running_data if record.get("status") == "failed"]
            if failed_records:
                latest_failed = failed_records[0]
                return StrategyStatusAnomalyReason.SCHEDULE_FAILED.label.format(
                    error_msg=latest_failed.get('err_msg', 'unknown error')
                )
...
def check_strategy_status_anomalies():
    ...
    1. Flow状态与策略状态不匹配
    2. 运行中策略缺少flow_id
    3. 策略失败状态信息（启动失败、更新失败、停止失败、删除失败、失败）
    4. 离线策略运行中但无调度记录
    5. 离线策略调度失败
    """
