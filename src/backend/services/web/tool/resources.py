...
class ExecuteTool(ToolBase):
    ...
    def _normalize_for_compare(value):
        """
        - 列表类型：排序后返回，屏蔽用户传入顺序差异（例如 [1,2] 与 [2,1] 视为等价）；
        - 其他类型：原样返回。
        仅用于比较判等，不改变原始数据。
        """
    ...
    def _validate_single_variable(self, raw_name, value, input_var_config_map, allowed_defaults, has_uncovered_scope):
        ...
        # 对列表类型默认值进行排序，避免因元素顺序不同导致误判
        cmp_value = self._normalize_for_compare(value)
        cmp_original = self._normalize_for_compare(original_default)
        cmp_allowed = [self._normalize_for_compare(v) for v in allowed_defaults.get(raw_name, [])]

        # 根据覆盖情况校验
        if raw_name not in allowed_defaults:
            # 无覆盖配置，只能用原始默认值
            if cmp_value != cmp_original:
                self._raise_permission_exception(raw_name, use_original_message=True)
        elif has_uncovered_scope:
            # 有无覆盖的场景/系统，允许：覆盖值 OR 原始默认值
            if cmp_value not in cmp_allowed and cmp_value != cmp_original:
                self._raise_permission_exception(raw_name)
        else:
            # 所有场景/系统都有覆盖，只能用覆盖值
            if cmp_value not in cmp_allowed:
                self._raise_permission_exception(raw_name)
